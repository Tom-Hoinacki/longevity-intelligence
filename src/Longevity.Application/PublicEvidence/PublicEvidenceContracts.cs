...
public sealed record PublicEvidenceClaim(Guid Id, Guid AssetId, string ClaimText, string? ClaimType, string? TargetSystem, decimal? EvidenceScore, string? EvidenceScoringPolicyId, decimal? HypeScore, decimal? RiskScore, string? PlainEnglishVerdict, int EvidenceCount, IReadOnlyList<PublicEvidenceItem> Evidence);
