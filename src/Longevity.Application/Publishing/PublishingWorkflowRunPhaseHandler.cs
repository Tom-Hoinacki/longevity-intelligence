...
public sealed class PublishingWorkflowRunPhaseHandler : IWorkflowRunPhaseHandler
{
    ...
    private static void Validate(ClaimedWorkflowRun run, ApprovedPublicationBatch batch, DateTimeOffset now)
    {
        ...
        if (batch.Claims.Any(c => !batch.EvidenceLinks.Any(e => e.CandidateId == c.CandidateId && e.SourceRecordId == batch.Source.SourceRecordId))) throw new PublicationInvariantException("Publication provenance is incomplete.");
    }
}
