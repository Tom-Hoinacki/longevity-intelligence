...
public sealed class ScientificSourceNormalizerTests
{
    ...
    [Fact] public void Invalid_identity_or_empty_content_is_rejected() { Assert.Throws<ArgumentException>(() => new ScientificSourceNormalizer().Normalize(Request(new(Pmid: "0")))); Assert.Throws<ArgumentException>(() => new ScientificSourceNormalizer().Normalize(new("x", "title", " ", new(CanonicalUrl: "https://x")))); }
    [Fact] public void Text_rules_preserve_scientific_content() { var r = new ScientificSourceNormalizer().Normalize(new("x", "\uFEFF T\u0301 ", "a\r\nb\r\n\r\n\r\n\r\n\tc\u00A0\u0001", new(CanonicalUrl: "https://x"))); Assert.Equal("T́", r.Title); Assert.Equal("a\nb\n\n\n\tc", r.NormalizedText); }
